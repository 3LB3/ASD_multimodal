# python main.py --GC_mode weighted-cosine --MF_mode concat --MP_mode GAT --dropout 0.35 --lr 0.0038 --mode simple-train --n_head 2 --n_hidden 18 --nlayer 1 --reg 0.11
...
def train_and_eval(datadir, datname, hyperpm):
    ...
    modal_feat_dict = np.load(path + 'dict_800.npy', allow_pickle=True).item()
    data = pd.read_csv(path + 'standard_model_800.csv').values
    ...
    return np.array(tst_acc).mean(), np.array(tst_acc).std(), np.array(tst_auc).mean(), np.array(tst_auc).std(),
...
def main(args_str=None):
    ...
    with RedirectStdStreams(stdout=sys.stderr):
        ...
        tst_acc, tst_acc_std, tst_auc, tst_auc_std = train_and_eval(args.datadir, args.datname, args)
        print('tst_acc=%.2f%% tst_auc=%.2f%%' % (tst_acc * 100, tst_auc * 100))
